...
import {
  ...
  useActiveExhibitionId,
} from '@/lib/active-exhibition';
import { ExhibitionSwitcher } from '@/components/exhibition-switcher';
...
export default function CaptureScreen() {
  ...
  // Make sure the audio session is configured for recording. Required on Android
  // for expo-audio's recorder to enter a valid state.
  useEffect(() => {
    AudioModule.setAudioModeAsync({
      allowsRecording: true,
      playsInSilentMode: true,
    }).catch((e) => console.warn('[record] setAudioModeAsync', e));
  }, []);

  const { data: exhibitions } = useExhibitions();
  const activeId = useActiveExhibitionId();
  // If saved active id no longer exists (deleted), fall back to most recent.
  const activeExhibition =
    exhibitions?.find((e) => e.id === activeId) ?? exhibitions?.[0];
  ...
  // Reset group + last-artifact state when the active exhibition changes.
  // Otherwise the next photo would (a) keep accumulating "本组 N 张" across
  // exhibitions, and (b) a voice recording could bind to an artifact from
  // the previous exhibition.
  const lastExhibitionIdRef = useRef<string | undefined>(undefined);
  useEffect(() => {
    const id = activeExhibition?.id;
    if (!id) return;
    if (
      lastExhibitionIdRef.current &&
      lastExhibitionIdRef.current !== id
    ) {
      console.log('[capture] exhibition changed, resetting group + last-artifact');
      activeGroupIdRef.current = null;
  ...
  const startRecording = async () => {
    // Reentrancy guard: rapid presses or React Compiler-induced re-fires
    // shouldn't kick off a second concurrent record. Also bail if we're
    // already recording per native state.
    if (recordingBusyRef.current || recorderState.isRecording) {
      console.log('[record] start ignored (already busy)');
      return;
    }
    ...
    try {
      ...
      try {
        ...
        // "already prepared" is benign — recorder is already in a recordable state.
        if (!msg.toLowerCase().includes('already')) throw prepErr;
      }
      // record() is typed void but actually returns a Promise that can reject
      // (e.g. IllegalStateException). Await so we catch native rejections here.
      await (recorder.record() as unknown as Promise<void> | void);
      recordingStartRef.current = Date.now();
      console.log('[record] started');
    } catch (e) {
      console.error('[record] start error', e);
      const msg = e instanceof Error ? e.message : String(e);
      const hint = msg.toLowerCase().includes('illegalstate')
        ? '\n\n录音器状态异常。建议杀掉 App 重开后再试。'
        : '';
      Alert.alert('录音启动失败', msg + hint);
    } finally {
  ...
  return (
    <View className="flex-1 bg-black">
      ...
      <ExhibitionSwitcher
        ...
        selectedId={activeExhibition.id}
        onSelect={(id) => {
          setActiveExhibitionId(id).catch((e) =>
            console.error('[active-exhibition]', e),
          );
